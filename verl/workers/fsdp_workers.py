...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            ...
            if torch.distributed.get_world_size() == 1:
                # Respect user override if load_format is explicitly set.
                if self.config.rollout.get('load_format', None) in (None, 'auto'):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_prob(self, data: DataProto,return_max: bool = False):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        # allow passing control flags via meta_info due to DP_COMPUTE_PROTO dispatch constraints
        return_topk_flag = data.meta_info.get('return_topk', False)
        topk = data.meta_info.get('top_k', 1)
        # print('topk',topk)
        # print('return_topk_flag',return_topk_flag)
        # perform recompute log_prob
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output = self.actor.compute_log_prob(data=data,simko=return_topk_flag,top_k=topk)
            ...
            elif return_topk_flag:
                output = DataProto.from_dict(tensors={'old_log_probs': output[0],'old_log_probs_topk': output[2]},
                                             meta_info={'temperature': self.config.rollout.temperature})
            else:
