...
def ensure_numpy(x):
    if isinstance(x, torch.Tensor):
        ...
        raise TypeError(f"Unsupported type {type(x)} in chunking: {x}")
def to_serializable(obj):
    ...
    elif hasattr(obj, '__dict__'):  # 处理 TensorDict 或其他自定义类
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('old_log_prob', timing_raw):
                        batch.meta_info['log_prob_key'] = 'old_log_probs'
                        batch.meta_info['return_topk'] = self.config.actor_rollout_ref.actor.get('simko', False)
                        batch.meta_info['top_k'] = self.config.actor_rollout_ref.actor.get('top_k', 1)
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        with _timer('reward_fn', timing_raw):
                            ...
                            prompt_str_list = reward_result['prompt_str']
                            response_str_list = reward_result['response_str']
                            score_list = reward_result['score']
                            all_prompt_str_list = reward_result['all_prompt_str_list']
                        batch.batch['token_level_scores'] = reward_tensor
                        
                        batch.non_tensor_batch['all_prompt_str_list'] = all_prompt_str_list
                        ...
                        incorrect_idx = (batch.batch['token_level_scores'].sum(-1) <= 0) | (batch.batch['token_level_scores'].sum(-1) == -1)
